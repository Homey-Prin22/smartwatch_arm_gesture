...
class AcquisitionActivity : Activity(), SensorEventListener {
    ...
    private val sensorReadingInterval = 5000L // 5L  // intervallo di lettura in millisecondi
    ...
    private fun readSensorValues() {
        ...
        val jsonString = Json.encodeToString(organizedSensorData)
        println(jsonString)

        //val file = File(this.filesDir, "prova.json")
        //file.writeText(jsonString)

        ModelHelpers.saveStringToAPI(jsonString,"smart_watch_acquisition")
        ...
        /*if(allData.size >= 500) {
            ...
            if(count >= 37) {
                ...
                mResultView.text = "Data transfer completed"
    ...
    private fun startMonitoring() {
        ...
        barometer?.let { sensorManager.registerListener(this, it, 100000) }
        temperature?.let { sensorManager.registerListener(this, it, 100000) }
        humidity?.let { sensorManager.registerListener(this, it, 100000) }
        lightSensor?.let { sensorManager.registerListener(this, it, 100000) }
    ...
    private fun pauseMonitoring() {
        ...
        backgroundRoot.setBackgroundColor(resources.getColor(android.R.color.black))
        mResultView.setTextColor(resources.getColor(android.R.color.white))
        mResultView.text="Tap to fetch data"
    }
